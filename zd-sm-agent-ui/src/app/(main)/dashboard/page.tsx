...
/**
 * ARCHITECT C - DASHBOARD V1
 * All fixes implemented:
 * 1. ✅ Badge redesign (Blue/Orange/Purple)
 * 2. ✅ Overflow fix (dropdown not cut off)
 * 3. ✅ Published icons only (with tooltips)
 * 4. ✅ View button always visible
 * 5. ✅ Remove duplicate delete
 * 6. ✅ Convert to Post modal
 * 7. ✅ Cross-posting support
 */
...
import {
  ...
  Trash2, MoreVertical, Grid3x3, Play,
...
import { ConvertToPostModal } from '@/components/dashboard/ConvertToPostModal';
  ...
  switch (sourceType) {
    case 'social_post':
      return {
        ...
        icon: <Grid3x3 className="w-4 h-4 mr-1" />,
        label: 'Social Post'
      };
...
const CarouselCard: React.FC<{
  ...
  return (
    <motion.div
      ...
      className="bg-[#10101d] rounded-xl shadow-lg border border-gray-800 flex flex-col"
      style={{ aspectRatio: '4/5' }}
    >
      {/* Image Section - Top 60% */}
      <div className="h-3/5 relative overflow-hidden rounded-t-xl">
        <img
          src={currentPost.image_url || 'https://placehold.co/400x500/10101d/5ccfa2?text=No+Image'}
        ...
        {hasMultiple && (
          <>
            <button
              onClick={(e) => { e.stopPropagation(); setCurrentIndex((prev) => (prev - 1 + group.posts.length) % group.posts.length); }}
              className="absolute left-2 top-1/2 -translate-y-1/2 bg-black/50 hover:bg-black/70 text-white p-2 rounded-full"
            >
              <ChevronLeft className="w-4 h-4" />
            </button>
            <button
              onClick={(e) => { e.stopPropagation(); setCurrentIndex((prev) => (prev + 1) % group.posts.length); }}
          ...
          <div className="mt-2">
            ...
            <div className="flex items-center justify-between">
              <div className="flex space-x-2">
                {group.posts.filter(p => p.published).map(post => (
                  <a
                    ...
                    title={`Click to view on ${PLATFORM_NAMES[post.platform]}`}
              ...
              <button
                ...
                className="px-3 py-1.5 bg-gray-800 hover:bg-gray-700 text-white text-xs rounded-lg transition-colors flex items-center"
              >
    ...
    >
      ...
      <div className="h-2/5 p-4 flex flex-col justify-between">
        ...
        <div className="flex items-center space-x-2 mt-2">
          {isImage && (
            <button
              onClick={() => onConvertToPost(post)}
              className="flex-1 px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs rounded-lg transition-colors flex items-center justify-center"
            >
      ...
      >
        ...
        >
          ...
          <div className="flex items-center justify-between p-4 border-b border-gray-800 sticky top-0 bg-[#0b0b10] z-10">
            ...
            <button onClick={onClose} className="px-3 py-1 rounded bg-transparent hover:bg-gray-800 transition-colors">
              Close
            </button>
          </div>
...
const PublishModal: React.FC<{
  ...
  const handlePublish = async () => {
    ...
    try {
      // 1. Check which platforms need new rows
      ...
      // 2. Create missing rows if needed
      let allPostIds = group.posts
      ...
      const payload: any = {
        ...
        li_publish: null,
        tt_publish: null,
        userId: group.posts[0].user_id,
      };

      // Map post IDs to platforms
      ...
      >
        ...
        >
          ...
          <div className="flex items-center justify-end space-x-3 p-4 border-t border-gray-800">
            <button onClick={onClose} className="px-4 py-2 rounded bg-gray-800 hover:bg-gray-700 text-sm transition-colors">
              Cancel
            </button>
            <button
    ...
    return (
      <div className="flex justify-center items-center min-h-[60vh]">
        <div className="bg-red-900/20 border border-red-700 p-8 rounded-xl max-w-lg text-center">
          ...
          <button onClick={refetch} className="mt-6 px-4 py-2 bg-red-600 rounded-lg hover:bg-red-700">
            Retry
          </button>
        </div>
      </div>
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="flex justify-between items-center pb-4 border-b border-gray-800">
        ...
        <button onClick={refetch} className="p-2 rounded-full text-gray-400 hover:text-[#5ccfa2]">
      ...
      {convertModalOpen && selectedPost && (
        <ConvertToPostModal
          ...
          onClose={() => { setConvertModalOpen(false); setSelectedPost(null); }}
          onSuccess={refetch}
        />
      )}
    </div>
